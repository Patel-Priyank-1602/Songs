...
function loadLikes() {
  try {
    ...
    if (savedLikes) {
      ...
      // Update songs array with saved likes
      songs.forEach((song, index) => {
        if (likesData[song.title]) {
          songs[index].likes = likesData[song.title]
        }
      })
    }
  } catch (error) {
...
function saveLikes() {
  try {
    const likesData = {}
    songs.forEach((song) => {
      likesData[song.title] = song.likes
    })
...
// Initialize
loadLikes() // Load likes before initializing
originalSongs = [...songs]
filteredSongs = [...songs]
  ...
  filteredSongs.forEach((song, index) => {
    ...
    playlistItem.innerHTML = `
            ...
            <div class="playlist-item-like">
                <button class="like-button ${song.likes > 0 ? "liked" : ""}" data-title="${song.title}">
...
function handleLike(e) {
  ...
  const songIndex = originalSongs.findIndex((song) => song.title === songTitle)

  if (songIndex !== -1) {
    // Increment like count
    originalSongs[songIndex].likes += 1

    // Update UI
    const likeCount = this.nextElementSibling
    likeCount.textContent = originalSongs[songIndex].likes
    this.classList.add("liked")

    // Update filtered songs if the song is in the current filtered list
    const filteredIndex = filteredSongs.findIndex((song) => song.title === songTitle)
    if (filteredIndex !== -1) {
      filteredSongs[filteredIndex].likes = originalSongs[songIndex].likes
    }
...
function loadSongDuration(index) {
  ...
  tempAudio.crossOrigin = "anonymous" // Add CORS header
...
function toggleMute() {
  if (audio.volume > 0) {
    ...
    volume = Number.parseFloat(audio.dataset.previousVolume || 0.7)
    audio.volume = volume
  }
...
function filterPlaylist() {
  ...
  if (searchTerm === "") {
    ...
    filteredSongs = languageFiltered.filter(
  ...
  const currentSongInFiltered = filteredSongs.findIndex(
    (song) =>
      song.title === originalSongs[currentSongIndex].title && song.artist === originalSongs[currentSongIndex].artist,
  )
  ...
  if (currentSongInFiltered === -1 && filteredSongs.length > 0) {
    ...
    playlistItems.forEach((item) => {
      item.classList.remove("active")
      if (Number.parseInt(item.dataset.index) === currentSongIndex) {
        item.classList.add("active")
      }
    })
  }
